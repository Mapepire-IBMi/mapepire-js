...
test("Selecting large CLOB as prepared", { timeout: 999999 }, async () => {
  const TABLE_NAME = "SAMPLE.MY_CLOB_TABLE";
  ...
  const stmt1 = job.query<any[]>(`
    INSERT INTO ${TABLE_NAME} (TXT_DATA)
    VALUES (?)
  `, {
    parameters: [str],
  ...
  const res = await job.execute<any>(`SELECT * FROM ${TABLE_NAME}`);
  expect(res.data[0].TXT_DATA.substring(0, 100)).toStrictEqual(str.substring(0, 100));
  expect(res.data[0].TXT_DATA.length).toEqual(sizeInChars);
  await job.close();
});
...
test("inserting blob as prepared, with null blob",{timeout: 999999}, async () => {
  const TABLE_NAME = "SAMPLE.MY_BLOB_TABLE";
  const text = "HELLO";
...
test("Selecting small BLOB as prepared multiple rows", async () => {
  const TABLE_NAME = "SAMPLE.MY_BLOB_TABLE";
  const text = "HELLO";
  const text2 = "GOODBYE123#@!&*"

  // Step 1: Convert to a Uint8Array (binary representation)
  const encoder = new TextEncoder();  // defaults to UTF-8
  const text1Encoded = encoder.encode(text);  
  const text2Encoded = encoder.encode(text);  
  ...
  const res = await job.execute<any>(`SELECT * FROM ${TABLE_NAME}`);
  expect(res.data[0].BIN_DATA).toStrictEqual(new Buffer(text1Encoded));
  expect(res.data[1].BIN_DATA).toStrictEqual(new Buffer(text2Encoded));
